...
const getTodayStr = () => new Date().toLocaleDateString("en-CA"); // YYYY-MM-DD format (local time)
...
function AdminScreen({ isReady, onBack }) {
  ...
  const applyFilters = (rawDocs) => {
    let data = rawDocs;
    if (filterDate) {
      const selectedStart = new Date(filterDate); selectedStart.setHours(0, 0, 0, 0);
      const selectedEnd = new Date(selectedStart);
      if (dateMode === "WEEK") selectedEnd.setDate(selectedEnd.getDate() + 7); else selectedEnd.setDate(selectedEnd.getDate() + 1);
      data = data.filter((d) => { if (!d.timestamp) return false; const scanTime = d.timestamp.toDate(); return scanTime >= selectedStart && scanTime < selectedEnd; });
    }
    return data;
  };
  ...
  const calculateTurns = (filteredData) => {
    const minMap = {};
    filteredData.forEach((d) => { if (d.status === "waiting" || d.status === "active") { if (!minMap[d.locationId] || d.queueNumber < minMap[d.locationId]) minMap[d.locationId] = d.queueNumber; } });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // ✅ 2. THEN CALCULATE TURNS (Now it ignores ghosts from yesterday)
      ...
      tableData.sort((a, b) => (b.timestamp?.toMillis() || 0) - (a.timestamp?.toMillis() || 0));
      setScans(tableData);
    });
    return () => unsubscribe();
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b"><tr><th className="px-6 py-4">Loc</th><th className="px-6 py-4">#</th><th className="px-6 py-4">User</th><th className="px-6 py-4">Time</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Action</th></tr></thead>
                    ...
                    <td className="px-6 py-4 flex gap-2 items-center">
                      {(s.status === "waiting" || s.status === "active") && (
                        <>
                          ...
                          {activeTurnMap[s.locationId] === s.queueNumber && <div className="flex items-center px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full animate-pulse whitespace-nowrap"><CheckCircle size={12} className="mr-1" /> NOW</div>}
                        </>
                      )}
                    </td>
                  </tr>
...
function ScannerScreen({ token, locationId, isReady, user }) {
  ...
  useEffect(() => {
    if (status === "success" && myQueueNumber && !isCheckedOut && isReady && db) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        // ✅ CRITICAL: Filter "People Ahead" by today's date only!
        const count = snapshot.docs
          .map((d) => d.data())
          .filter((user) => user.queueNumber < myQueueNumber && user.date === todayStr)
          .length;
  ...
  const saveCheckIn = async (coords) => {
    ...
    try {
      ...
      // ✅ ADDING DATE TO TICKET FOR FILTERING
      const todayStr = getTodayStr(); 
      ...
      const qNum = await runTransaction(db, async (t) => {
        ...
        t.set(newRef, { 
          ...
          date: todayStr // <--- Critical Addition
        });
        return next;
      });
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-green-50 text-center animate-in zoom-in relative">
        ...
        <div className="w-full max-w-xs mb-8">
          <div className={`${peopleAhead === 0 ? "bg-green-600" : "bg-blue-600"} text-white p-4 rounded-xl shadow-md transition-colors`}>
            ...
            <div className="text-4xl font-bold">{peopleAhead === 0 ? "It's your turn!" : formatNum(peopleAhead)}</div>
          </div>
        </div>
