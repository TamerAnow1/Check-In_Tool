...
function AdminScreen({ isReady, onBack }) {
  ...
  const applyFilters = (rawDocs) => {
    ...
    if (filterDate) {
      ...
      if (dateMode === "WEEK") selectedEnd.setDate(selectedEnd.getDate() + 7);
      else selectedEnd.setDate(selectedEnd.getDate() + 1);
      data = data.filter((d) => {
                    ...
                    <td className="px-6 py-4 flex gap-2 items-center">
                      {(s.status === "waiting" || s.status === "active") && (
                        <>
                          ...
                          {activeTurnMap[s.locationId] ===
                            Number(s.queueNumber) && (
