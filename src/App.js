...
function AdminScreen({ isReady, onBack }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      calculateTurns(dateFilteredData); // ✅ Now uses unified sort
                    ...
                    <td className="px-6 py-4 flex gap-2 items-center">
                      {(s.status === "waiting" || s.status === "active") && (
                        <>
                          ...
                          {activeTurnMap[s.locationId] === Number(s.queueNumber) && <div className="flex items-center px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full animate-pulse whitespace-nowrap"><CheckCircle size={12} className="mr-1" /> NOW</div>}
                        </>
                      )}
                    </td>
                  </tr>
