...
function AdminScreen({ isReady, onBack }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // SORT: Waiting first, then history
...
function ScannerScreen({ token, locationId, isReady, user }) {
  ...
  const saveCheckIn = async (coords) => {
    setStatus("saving");
    try {
      // ✅ 1. CHECK FOR DUPLICATES (Device ID)
      const deviceQ = query(
        collection(db, COLLECTION_NAME),
      ...
      // ✅ 4. CREATE NEW TICKET IF NONE EXIST
      const todayStr = getTodayStr();
